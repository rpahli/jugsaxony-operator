...
//+kubebuilder:rbac:groups=demo.jugsaxony.org,resources=games,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=demo.jugsaxony.org,resources=games/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=demo.jugsaxony.org,resources=games/finalizers,verbs=update
...
func (r *GameReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ret ctrl.Result, reterr error) {
	...
	gameFinalizerName := "demo.jugsaxony.org/finalizer"
	...
	guessList := &jugsaxonyv1beta1.GuessList{}
	...
	if err := r.Client.Get(ctx, game.Spec.Solution.ToObjectKey(), phrase); err != nil {
		if apierrors.IsNotFound(err) {
			...
			game.Spec.Solution = jugsaxonyv1beta1.NamespacedName(client.ObjectKeyFromObject(phrase))

			kind := reflect.TypeOf(jugsaxonyv1beta1.Game{}).Name()
			gvk := jugsaxonyv1beta1.GroupVersion.WithKind(kind)
			controllerRef := metav1.NewControllerRef(game, gvk)
			phrase.SetOwnerReferences([]metav1.OwnerReference{*controllerRef})
	...
	r.Client.List(ctx, guessList, &client.MatchingLabels{"jug-game": game.Name})

	return ctrl.Result{}, reterr
}

func (r *GameReconciler) reconcilePhase(game *jugsaxonyv1beta1.Game, guesses *[]jugsaxonyv1beta1.Guess, phrase string) {
	...
	if game.Status.Phase == "" {
		game.Status.SetTypedPhase(jugsaxonyv1beta1.GamePhasePending)
	}

	if game.Status.NumberOfGuesses > 0 {
		game.Status.SetTypedPhase(jugsaxonyv1beta1.GamePhaseActive)
	}

	if game.Status.NumberOfGuesses > MaxGuesses {
		game.Status.SetTypedPhase(jugsaxonyv1beta1.GamePhaseFinished)
	}
...
func (r *GameReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&jugsaxonyv1beta1.Game{}).
		Watches(
			&source.Kind{Type: &jugsaxonyv1beta1.Guess{}},
...
func (r *GameReconciler) GuessToGame(o client.Object) []ctrl.Request {
	...
	guess, ok := o.(*jugsaxonyv1beta1.Guess)
	if !ok {
...
func (r *GameReconciler) deleteExternalResources(game *jugsaxonyv1beta1.Game) error {
	//
	...
	guessList := &jugsaxonyv1beta1.GuessList{}
	r.Client.List(context.Background(), guessList, &client.MatchingLabels{"jug-game": game.Name})
